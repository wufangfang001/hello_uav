...
typedef struct rkVENC_JPEG_ROI_ATTR_S {
    /* RW; Range:[0, 15]; Index of an ROI. The system supports indexes ranging from 0 to 15 */
    RK_U32  u32Index;
    RECT_S  stRect;                       /* RW; Region of an ROI*/
    /* RW; Range:[0, 1]; Whether to enable this ROI */
    RK_BOOL bRoiEnable;
    RK_U32  u32RoiLevel;                  /* RW; Range:[0, 63]; Quality level of an ROI */
    /* RW; Range:[0, 1]; Whether to enable non ROI */
    RK_BOOL bNonRoiEnable;
    RK_U32  u32NonRoiLevel;            /* RW; Range:[0, 63]; Quality level of a non-ROI */
...
typedef struct rkVENC_H265_SAO_S {
    ...
    RK_U32  u32SaoStrengthI;
    RK_U32  u32SaoStrengthP;
} VENC_H265_SAO_S;
...
typedef struct rkUSER_NPU_INFO_S {
    RK_BOOL bNpuValid;
    void *ptr;
} USER_NPU_INFO_S;
...
typedef struct rkVENC_ANTI_RING_S {
    ...
    RK_U32 u32AntiRingI; /* RW; Range:[0, 3] */
} VENC_ANTI_RING_S;
...
typedef struct rkVENC_SVC_PARAM_S {
    RK_U32 u32FrmMaxPQp;   // RW; Range:[u32FrmMinPQp, 51];max qp for p frame;default : 41
    RK_U32 u32FrmMinPQp;   // RW; Range:[0, u32FrmMaxPQp];min qp for p frame;default : 15
    RK_U32 u32FrmMaxIQp;   // RW; Range:[u32FrmMinIQp, 51];max qp for i frame;default : 43
    RK_U32 u32FrmMinIQp;   // RW; Range:[0, u32FrmMaxIQp];min qp for i frame;default : 15
} VENC_SVC_PARAM_S;

typedef struct rkVENC_SVC3_PARAM_S {
    RK_U32 u32BgFrmMinQp;  // RW; Range:[0, 51];min qp for background frame
    RK_U32 u32BgFrmMaxQp;  // RW; Range:[0, 51];max qp for background frame
    RK_U32 u32FgFrmMinQp;  // RW; Range:[0, 51];min qp for foreground frame
    RK_U32 u32FgFrmMaxQp;  // RW; Range:[0, 51];max qp for foreground frame
    RK_U32 u32BmapMinIQp;  // RW; Range:[0, 51];min qp for i frame qpmap
    RK_U32 u32BmapMaxIQp;  // RW; Range:[0, 51];max qp for i frame qpmap
    RK_U32 u32BmapMinPQp;  // RW; Range:[0, 51];min qp for p frame qpmap
    RK_U32 u32BmapMaxPQp;  // RW; Range:[0, 51];max qp for p frame qpmap
    RK_S32 s32BgDeltaIQp;  // RW; Range:[-32, 32];delta qp for background i frame
    RK_S32 s32BgDeltaPQp;  // RW; Range:[-32, 32];delta qp for background p frame
    RK_S32 s32FgDeltaIQp;  // RW; Range:[-32, 32];delta qp for foreground i frame
    RK_S32 s32FgDeltaPQp;  // RW; Range:[-32, 32];delta qp for foreground p frame
    RK_S32 s32FgArea;      // RW; Range:[-1, 99];foreground area
} VENC_SVC3_PARAM_S;

typedef enum rkVENC_SUPER_ENCODE_MODE {
    SUPER_ENCODE_BALANCE = 1,
    SUPER_ENCODE_QUALITY_FIRST,
    SUPER_ENCODE_BITRATE_FIRST,
    SUPER_ENCODE_EXTERNAL,
    SUPER_ENCODE_BUTT
} VENC_SUPER_ENCODE_MODE;

#define VENC_MAX_OBJ_NUM 128

typedef struct rkVENC_OBJ_DETECT_INFO_S {
    RECT_S stRect;
    RK_U32 u32Level;
    RK_U32 u32Res[7];
} VENC_OBJ_DETECT_INFO_S;

typedef struct rkVENC_OBJ_DETECT_S {
    RK_U32 u32FrameId;
    RK_U32 u32ObjNum;
    VENC_OBJ_DETECT_INFO_S stObjInfo[VENC_MAX_OBJ_NUM];
} VENC_OBJ_DETECT_S;
