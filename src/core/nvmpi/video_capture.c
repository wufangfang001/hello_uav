...
int video_capture_start(void)
{
  ...
  LOGT(TAG, "video capture start success.");
  return 0;
}

static int __camera_set_fps(uint32_t fps)
{
...
static int __camera_set_video_fmt(uint32_t width, uint32_t height)
{
...
int video_capture_init(uint32_t width, uint32_t height, uint32_t fps)
{
  ...
  if (0 != (ret = __camera_set_video_fmt(width, height))) {
    LOGE(TAG, "camera_set_video_fmt error!");
    return ret;
  }

#if 0 /** 当前使用的摄像头不支持设置帧率，默认25帧 */
  if (0 != (ret = __camera_set_fps(fps))) {
  ...
  LOGT(TAG, "video capture init success.");
  return ret;
}
...
int video_capture_stop(void)
{
  ...
  LOGT(TAG, "video capture stop success.");
  return 0;
}
...
void video_capture_fini(void)
{
  ...
  LOGT(TAG, "video capture fini success.");
}
