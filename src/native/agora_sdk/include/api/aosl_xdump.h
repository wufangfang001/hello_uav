 ...
 *   @extra_info: extra info bits for providing more information
 *                after this struct, please see the followings;
 **/
...
typedef struct {
    ...
    void *ld_end;
    int q_flags;
#define AOSL_XDUMP_EXI_STALLED_MPQS 0x01 /* aosl_xdump_qids_t */
    uint32_t extra_info;
} xdump_info_t;

typedef void (*xdump_cb_t)(const xdump_info_t *info);

/**
 * if the extra_info field of xdump info contains AOSL_XDUMP_EXI_STALLED_MPQS,
 * then the following struct is at "info + 1", which provides the stalled qids
 * information.
 **/
typedef struct {
    size_t count;
    const aosl_mpq_t *qids;
} aosl_xdump_qids_t;
