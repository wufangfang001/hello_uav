 ...
 *    exec_ts: the executing timestamp of the function(microseconds)
 *    wait_us: the wait time in queue before executing(microseconds)
 *    exec_us: the executing time(microseconds)
 * Return Value:
 *    none.
 **/
typedef void (*aosl_perf_f_t) (const char *f_name, int free_only, aosl_ts_t exec_ts, uint32_t wait_us, uint32_t exec_us, uint32_t extra_info, ...);
