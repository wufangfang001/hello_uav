...
public:
	...
	typename std::enable_if<!std::is_void<__ret_t_>::value, __ret_t_>::type operator () (__arg_types_... args) const
	{
		if (__obj_ != NULL)
	...
	template <typename __ret_t_ = __ret_type_>
	typename std::enable_if<std::is_void<__ret_t_>::value, void>::type operator () (__arg_types_... args) const
	{
		if (__obj_ != NULL) {
	...
	aosl_callable (aosl_callable &&src)
	{
		if (this != &src) {
			...
			/* Need to clear gf for both cases. */
			src.__gf_ = NULL;
		}
	}
