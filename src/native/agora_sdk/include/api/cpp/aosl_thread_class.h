...
class aosl_thread_class {
public:
	typedef aosl_callable<void (void)> aosl_thread_entry_lambda_f;

	static int create (const char *name, aosl_thread_entry_lambda_f &&entry)
	{
		aosl_thread_entry_lambda_f *entry_p = new aosl_thread_entry_lambda_f (std::move (entry));
		int err = aosl_thread_create (name, ____thread_entry, entry_p);
		if (err < 0)
			delete entry_p;
...
private:
	static void ____thread_entry (void *arg)
	{
		aosl_thread_entry_lambda_f *entry_p = (aosl_thread_entry_lambda_f *)arg;
		(*entry_p) ();
		delete entry_p;
	}

private:
	aosl_thread_class () = delete;
	aosl_thread_class (const aosl_thread_class &) = delete;
	aosl_thread_class (aosl_thread_class &&) = delete;
	aosl_thread_class &operator = (const aosl_thread_class &) = delete;
	aosl_thread_class &operator = (aosl_thread_class &&) = delete;
	~aosl_thread_class () = delete;
};
#endif /* C++11 */
