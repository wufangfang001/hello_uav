...
/**
 * Create a new thread with the specified name and entry function.
 * Parameters:
 *       name: the thread name
 *      entry: the thread entry function
 *        arg: the thread entry function argument
 * Return Value:
 *          0: success
 *         <0: failure with aosl_errno set
 **/
extern __aosl_api__ int aosl_thread_create (const char *name, aosl_thread_entry_t entry, void *arg);

extern __aosl_api__ aosl_thread_t aosl_thread_self (void);
extern __aosl_api__ aosl_thrdid_t aosl_thread_self_id (void);

/**
 * Get the specified thread name.
 * Parameters:
