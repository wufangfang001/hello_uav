...
int nvmpi_encoder_setFrameRate(nvmpictx* ctx, int fps_n, int fps_d){
	ctx->enc->setFrameRate(fps_n, fps_d);
	return 0;
}
...
int nvmpi_encoder_setBitrate(nvmpictx* ctx, int bps){
	ctx->enc->setBitrate(bps);
	return 0;
}
